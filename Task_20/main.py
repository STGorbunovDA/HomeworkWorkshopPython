...
N = abs(int(input('1 - English, 2 - Русский: ' )))

str = input('Введите слово: ').upper()

if N == 1:
	print(f'За слово {str} получено {sum([k for i in str for k, v in eng.items() if i in v])} очков')
elif N == 0:
	print(f'За слово {str} получено {sum([k for i in str for k, v in rus.items() if i in v])} очков')
else:
    print('Тебе вечный Бан!')